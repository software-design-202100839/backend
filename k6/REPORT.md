...
### 로그인 병목 — 현 구성 유지
- 동시 50명이 쉬지 않고 로그인을 반복하는 시나리오는 비현실적
- 실 사용 패턴: 교사 수십 명이 아침에 각각 1번 로그인 → 이후 토큰으로 API 호출
- 필요 시 CPU 스케일업(512→1024)으로 즉시 대응 가능하므로 선제 조치 불필요

### Redis @Cacheable — 도입 불필요
- 조회 API(students, subjects)는 이미 수백ms 이내로 충분히 빠름
- 현재 규모(교사 수십 명, 학생 수백 명)에서 캐싱 없이도 성능 문제 없음
- 캐시 무효화 복잡도 대비 얻는 이점이 미미하므로 도입하지 않음

### 요약
부하 테스트를 통해 병목 지점(bcrypt + 제한된 CPU)을 식별했으나, 실 사용 패턴 분석 결과 현 구성으로 충분하다고 판단. 불필요한 최적화를 추가하지 않는 것이 올바른 설계 결정이다.
