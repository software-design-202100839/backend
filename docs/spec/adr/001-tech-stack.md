...
- 다중 학교 SaaS 확장 시: 학교당 교사 20~50명 × N개 학교 → 동시 수백~수천 명.
- Spring Boot 기본 Tomcat: 200 스레드 동시 처리 가능. 단일 인스턴스 기준.
- ECS Fargate Auto Scaling(1→3 태스크)으로 수평 확장. 시험 기간 여러 학교 동시 피크 대응.
- **동시 수정 충돌**: JPA `@Version`을 활용한 Optimistic Locking.
- **증명**: k6 부하 테스트로 200 VU(10개 학교 동시) 기준 p95 응답시간 791ms 달성.
